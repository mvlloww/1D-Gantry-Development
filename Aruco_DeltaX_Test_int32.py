...
def main():
    ...
    udp_ip = '127.0.0.1' #Oskar's Laptop
    udp_port = 50002
    send_format = 'int32'
            ...
            if items:
                ...
                if min_send_interval == 0.0 or (now - last_send_time) >= min_send_interval:
                    try:
                        ...
                        elif send_format == 'int32':
                            # send exact pixel deltaX as signed 32-bit integer (network order)
                            # convert to integer pixels
                            try:
                                val = int(round(float(best_deltaX)))
                            except Exception:
                                val = 0
                            # clamp to int32 range just in case
            ...
            if min_send_interval == 0.0 or (now - last_send_time) >= min_send_interval:
                try:
                    if send_format == 'raw_float':
                        ...
                        # use 0x7FFFFFFF as sentinel for NaN
